...
2020 07 16
0. 听了前天 振西的 Python 代码规范
1. 给云厨房项目 切了个分支  加上了 eslint  规则用的  prettire  standard  + 自定义  如果 全部自己定义  太多了  使用 standard 大部分 都很好了(重复引用  引用了 但是没有没有使用  使用了 没有定义 )  不喜欢的风格可以自己定义修改( function 后的空格)   架子都搭好了  往上添加
2. standard 有个规则是 连个数据比较 的时候必须使用  ==== 也就是全等  不能送 ==  js比较松散  很多隐身转换  1 == '1' 两个值比较比较时   一定要先做手动的类型转换 再用 === 比较
3. 废钢不加 eslint  怕有bug
4. 不用害怕 eslint   有神器 
4. 配置了 vs code  自动根据当前项目的 eslint 规则 保存时自动格式化代码  (能改的自动改 不改的  必须手动改 就像 git 的合并代码 一样 有冲突就自动改)
5. 配置了一套组件标准格式模板  配置了 vs code 的 代码片段  以后很方便的 项目 所有的组件保持一致  
4. 官网 有加新需求了 加动画  改样式
...
2020 12 07 建信基金
1. 搭建一个后台管理系统: 联邦学习审计工具 (目前2个页面已有原型图 还会有3个正在画)
2. 不需要部署 本地 docker 形式 
3. 接口 Http(一般数据) 加 websocket (后端推送日志)
4. 后端和前端同步开始开发
